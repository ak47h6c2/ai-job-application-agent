...
- Added a local Chrome extension in `browser-extension/` so logged-in job pages can be imported using the user's real browser session instead of the backend's unauthenticated HTTP request.
...
- 新增本地 Chrome 扩展 `browser-extension/`：登录后的岗位页面可以用用户真实浏览器会话导入，不再依赖后端未登录的 HTTP 请求。
