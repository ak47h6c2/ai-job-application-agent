...
const translations = {
  en: {
    ...
    mailPathBody: "Read recent job emails, open the job links automatically when possible, then draft for the best matches.",
    ...
    startScan: "Scan mail, read links, create drafts",
    scanning: "Scanning mail and reading job links...",
    refresh: "Refresh saved results",
    scanActionHint: "Creates a new report from the selected mail range, tries to read job links automatically, and stops before any external action.",
    refreshHint: "Reloads saved reports only. It does not scan your mailbox.",
    scanSuccess: "Job emails and available job pages have been analyzed.",
    ...
    leadSnippetBody: "This is the email lead. The agent now tries to read the linked job page automatically; if the site blocks it, use the logged-in browser fallback.",
  ...
  zh: {
    ...
    mailPathBody: "从近期邮件里找岗位线索，能打开的岗位链接会自动读取完整 JD，再给匹配度高的岗位生成草稿。",
    ...
    startScan: "扫描邮件、读取链接并生成草稿",
    scanning: "正在扫描邮箱并读取岗位链接...",
    refresh: "刷新历史结果",
    scanActionHint: "会按上面的日期重新读取邮件，自动尝试读取邮件里的岗位链接，并生成新的当前报告；不会自动发送邮件或投递。",
    refreshHint: "只重新加载已经保存的结果，不会读取邮箱。",
    scanSuccess: "邮件和可读取的岗位网页已分析完成。",
    scanError: "分析失败。",
    manualTitle: "添加岗位 JD",
    ...
    leadSnippetBody: "这是邮件里的岗位线索。系统会自动尝试读取链接里的完整 JD；如果网站拦截，再用登录浏览器继续读取。",
    ...
    try {
      const response = await fetch(`${API_BASE}/api/runs`, {
        ...
        body: JSON.stringify({
          ...
          auto_read_limit: Math.min(10, Math.max(1, topDrafts))
        })
      });
      if (!response.ok) throw new Error(await parseApiError(response));
